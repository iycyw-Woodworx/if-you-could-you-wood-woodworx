...
if(modal){const modalImg=modal.querySelector('img');document.querySelectorAll('[data-full]').forEach(card=>card.addEventListener('click',()=>{modalImg.src=card.dataset.full;modalImg.alt=card.dataset.alt||card.dataset.title||'';modal.classList.add('open')}));const close=()=>modal.classList.remove('open');modal.querySelector('.modal-close').addEventListener('click',close);modal.addEventListener('click',e=>{if(e.target===modal)close()});document.addEventListener('keydown',e=>{if(e.key==='Escape')close()})}
